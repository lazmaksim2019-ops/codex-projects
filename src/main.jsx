...
const isValidDate = (value) => value === '' || (/^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00`)))
...
function isProject(value) {
  return value !== null
    ...
    && statusOptions.includes(value.status)
    && ['Высокий', 'Средний', 'Низкий'].includes(value.priority)
    ...
    && ['teal', 'slate', 'blue', 'mint', 'aqua'].includes(value.color)
}

function normalizeProjects(value) {
  if (!Array.isArray(value)) return seedProjects
  return value.every(isProject) ? value : seedProjects
}


function loadProjects() {
  try { const saved = localStorage.getItem(STORAGE_KEY); if (!saved) return seedProjects; return normalizeProjects(JSON.parse(saved)) } catch { return seedProjects }
}
function Icon({ name, size = 20 }) {
  const paths = { search: <><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></>, plus: <path d="M12 5v14M5 12h14" />, grid: <><rect x="4" y="4" width="6" height="6" rx="1" /><rect x="14" y="4" width="6" height="6" rx="1" /><rect x="4" y="14" width="6" height="6" rx="1" /><rect x="14" y="14" width="6" height="6" rx="1" /></>, chevron: <path d="m6 9 6 6 6-6" />, close: <path d="m6 6 12 12M18 6 6 18" />, edit: <><path d="m4 16-.8 4.8L8 20l11-11-4-4L4 16Z" /><path d="m13.5 6.5 4 4" /></>, trash: <><path d="M4 7h16M10 11v6M14 11v6M6 7l1 14h10l1-14M9 7V4h6v3" /></>, check: <path d="m5 12 4 4L19 6" />, calendar: <><rect x="3" y="4" width="18" height="17" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /></>, sparkle: <path d="m12 3 1.5 5.5L19 10l-5.5 1.5L12 17l-1.5-5.5L5 10l5.5-1.5L12 3Z" /> }
...
function App() {
  ...
  useEffect(() => { if (!modalOpen) return undefined; const timer = setTimeout(() => titleRef.current?.focus(), 50); const close = (event) => { if (event.key === 'Escape' && !deletingId) { setModalOpen(false); setErrors({}); return } if (event.key !== 'Tab' || !modalRef.current) return; const focusable = [...modalRef.current.querySelectorAll('button:not([disabled]), input:not([disabled]), textarea:not([disabled]), select:not([disabled])')]; if (!focusable.length) return; const first = focusable[0]; const last = focusable[focusable.length - 1]; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus() } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus() } }; window.addEventListener('keydown', close); return () => { clearTimeout(timer); window.removeEventListener('keydown', close) } }, [modalOpen, deletingId])
  useEffect(() => { if (modalOpen || !returnFocusRef.current) return; const trigger = returnFocusRef.current; returnFocusRef.current = null; requestAnimationFrame(() => trigger.focus()) }, [modalOpen])
  ...
  function openCreate(event) { returnFocusRef.current = event.currentTarget; setEditingId(null); setForm(emptyForm); setErrors({}); setModalOpen(true) }
  function openEdit(project, event) { returnFocusRef.current = event.currentTarget; setEditingId(project.id); setForm(project); setErrors({}); setModalOpen(true) }
  ...
  return <div className="app-shell">
    <header className="topbar"><a className="brand" href="#top"><span className="brand-mark"><Icon name="sparkle" size={17} /></span><span>projects<span className="brand-dot">.</span></span></a><div className="topbar-actions"><div className="avatar">А</div><span className="user-name">Алексей</span></div></header>
    <main id="top" className="main-content">
      ...
      {filtered.length ? <div className="project-grid">{filtered.map((project, index) => <article className={`project-card ${project.color}`} key={project.id} style={{ '--delay': `${index * 45}ms` }}><div className="card-top"><span className={`status ${statusClass(project.status)}`}><i />{project.status}</span><button className="card-menu" aria-label={`Редактировать проект «${project.title}»`} onClick={(event) => openEdit(project, event)}><Icon name="edit" size={16} /></button></div><div className="card-art"><span className="art-glow" /><span className="art-shape shape-one" /><span className="art-shape shape-two" /><span className="art-symbol">✦</span></div><div className="card-body"><span className="category">{project.category}</span><h3>{project.title}</h3><p>{project.description}</p><div className="card-meta"><span><Icon name="calendar" size={15} />{formatDate(project.dueDate)}</span><span className={`priority ${priorityClass(project.priority)}`}>{project.priority}</span></div><div className="progress-line"><div><span>Прогресс</span><b>{project.progress}%</b></div><div className="progress-track"><i style={{ width: `${project.progress}%` }} /></div></div><div className="card-actions"><button onClick={(event) => openEdit(project, event)}><Icon name="edit" size={16} />Редактировать</button><button className="delete" onClick={() => setDeletingId(project.id)} aria-label="Удалить проект"><Icon name="trash" size={16} /></button></div></div></article>)}</div> : <div className="empty-state"><div className="empty-icon"><Icon name="search" size={25} /></div><h3>Ничего не нашли</h3><p>Попробуйте изменить запрос или фильтр.</p><button onClick={() => { setQuery(''); setFilter('Все проекты') }}>Сбросить фильтры</button></div>}</section>
    </main>
    {modalOpen && <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && closeModal()}><section ref={modalRef} className="modal" role="dialog" aria-modal="true" aria-labelledby="project-modal-title" aria-describedby="project-modal-description"><div className="modal-header"><div><span className="modal-kicker">{editingId ? 'Редактирование' : 'Новый проект'}</span><h2 id="project-modal-title">{editingId ? 'Редактировать проект' : 'Создать проект'}</h2></div><button className="icon-button" onClick={closeModal} aria-label="Закрыть форму"><Icon name="close" size={20} /></button></div><p id="project-modal-description" className="sr-only">Заполните обязательные поля и сохраните тестовый проект.</p><form onSubmit={saveProject}><div className="form-grid"><label className={errors.title ? 'field has-error' : 'field'}><span>Название проекта <b>*</b></span><input ref={titleRef} aria-invalid={Boolean(errors.title)} aria-describedby={errors.title ? 'title-error' : undefined} value={form.title} onChange={(event) => updateField('title', event.target.value)} placeholder="Например, новый лендинг" />{errors.title && <small id="title-error">{errors.title}</small>}</label><label className="field"><span>Категория</span><select value={form.category} onChange={(event) => updateField('category', event.target.value)}><option>Web</option><option>E-commerce</option><option>Mobile</option><option>Portfolio</option><option>Product</option></select></label><label className={errors.description ? 'field full has-error' : 'field full'}><span>Описание <b>*</b></span><textarea aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? 'description-error' : undefined} value={form.description} onChange={(event) => updateField('description', event.target.value)} placeholder="Что нужно сделать в рамках проекта?" rows="3" />{errors.description && <small id="description-error">{errors.description}</small>}</label><label className="field"><span>Статус</span><select value={form.status} onChange={(event) => updateField('status', event.target.value)}>{statusOptions.map((item) => <option key={item}>{item}</option>)}</select></label><label className="field"><span>Приоритет</span><select value={form.priority} onChange={(event) => updateField('priority', event.target.value)}><option>Высокий</option><option>Средний</option><option>Низкий</option></select></label><label className="field"><span>Дедлайн</span><input type="date" value={form.dueDate} onChange={(event) => updateField('dueDate', event.target.value)} /></label><label className={errors.progress ? 'field has-error' : 'field'}><span>Прогресс, %</span><input type="number" aria-invalid={Boolean(errors.progress)} aria-describedby={errors.progress ? 'progress-error' : undefined} min="0" max="100" value={form.progress} onChange={(event) => updateField('progress', event.target.value)} />{errors.progress && <small id="progress-error">{errors.progress}</small>}</label></div><div className="modal-footer"><button type="button" className="cancel" onClick={closeModal}>Отмена</button><button className="save" type="submit"><span className="cta-icon"><Icon name="check" size={17} /></span><span>{editingId ? 'Сохранить изменения' : 'Создать проект'}</span></button></div></form></section></div>}
